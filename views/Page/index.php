...
// Get page counts
$db = new Database();
$totalPages = $db->prepareSelect("SELECT COUNT(*) as count FROM pages")->fetch()['count'] ?? 0;
$activePages = $db->prepareSelect("SELECT COUNT(*) as count FROM pages WHERE is_active = 1")->fetch()['count'] ?? 0;
$parentPages = $db->prepareSelect("SELECT COUNT(*) as count FROM pages WHERE parent_page_id IS NULL")->fetch()['count'] ?? 0;
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title><?php echo APP_NAME; ?> - Page Management</title>
    ...
    <style>
        ...
        .stat-card .icon.primary { background: rgba(2, 132, 199, 0.1); color: var(--primary-color); }
        ...
        .btn-action:hover { border-color: var(--primary-color); color: var(--primary-color); background: rgba(2, 132, 199, 0.05); }
        ...
        .form-control:focus, .form-select:focus { border-color: var(--primary-color); box-shadow: 0 0 0 3px rgba(2, 132, 199, 0.1); }
        code { background: rgba(2, 132, 199, 0.1); color: var(--primary-color); padding: 0.125rem 0.375rem; border-radius: 4px; font-size: 0.8125rem; }
    </style>
</head>
<body>
    <div class="wrapper">
                    ...
                    <div class="page-header">
                        <div class="page-header-content">
                            <div class="title-section">
                                <h1><i class="fas fa-file-alt me-2"></i>Page Management</h1>
                                <p>Manage application pages and menu structure</p>
                            </div>
                        </div>
                    </div>
                    <div class="stats-row">
                        <div class="stat-card"><div class="icon primary"><i class="fas fa-file"></i></div><div class="details"><h3><?php echo $totalPages; ?></h3><p>Total Pages</p></div></div>
                        <div class="stat-card"><div class="icon success"><i class="fas fa-check-circle"></i></div><div class="details"><h3><?php echo $activePages; ?></h3><p>Active</p></div></div>
                        <div class="stat-card"><div class="icon secondary"><i class="fas fa-sitemap"></i></div><div class="details"><h3><?php echo $parentPages; ?></h3><p>Parent Pages</p></div></div>
                    </div>
                    <div class="data-card">
                        <div class="data-card-header">
                            <h5><i class="fas fa-list me-2"></i>Pages List</h5>
                            <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#pageModal"><i class="fas fa-plus me-2"></i>Add Page</button>
                        </div>
                        <div class="data-card-body">
                            <div class="table-responsive">
                                <table id="pagesTable" class="table table-hover" style="width:100%">
                                    <thead><tr><th>ID</th><th>Page Name</th><th>Route</th><th>Category</th><th>Description</th><th>Icon</th><th>Order</th><th>Active</th><th>Parent</th><th>Created At</th><th class="text-end">Actions</th></tr></thead>
                                    <tbody></tbody>
                                </table>
                            </div>
                        </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                <form id="pageForm" class="needs-validation" novalidate>
                    <div class="modal-header"><h5 class="modal-title"><i class="fas fa-file-medical me-2"></i>Add New Page</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                    <div class="modal-body">
                        <input type="hidden" id="page_id" name="id">
                        <div class="row"><div class="col-md-6 mb-3"><label for="page_name" class="form-label">Page Name <span class="text-danger">*</span></label><input type="text" class="form-control" id="page_name" name="page_name" required placeholder="Enter page name"></div><div class="col-md-6 mb-3"><label for="page_route" class="form-label">Page Route <span class="text-danger">*</span></label><input type="text" class="form-control" id="page_route" name="page_route" required placeholder="e.g., /dashboard"></div></div>
                        <div class="row"><div class="col-md-6 mb-3"><label for="page_category" class="form-label">Category</label><input type="text" class="form-control" id="page_category" name="page_category" placeholder="Enter category (optional)"></div><div class="col-md-6 mb-3"><label for="display_order" class="form-label">Display Order</label><input type="number" class="form-control" id="display_order" name="display_order" min="0" value="0"></div></div>
                        <div class="row"><div class="col-md-6 mb-3"><label for="parent_page_id" class="form-label">Parent Page</label><select class="form-select" id="parent_page_id" name="parent_page_id"><option value="">-- Select Parent (Optional) --</option></select></div><div class="col-md-6 mb-3"><label for="icon" class="form-label">Icon Class</label><input type="text" class="form-control" id="icon" name="icon" placeholder="e.g., fas fa-home"></div></div>
                        <div class="mb-3"><label for="description" class="form-label">Description</label><textarea class="form-control" id="description" name="description" rows="2" placeholder="Enter page description (optional)"></textarea></div>
                        <div class="form-check form-switch"><input class="form-check-input" type="checkbox" id="is_active" name="is_active" checked><label class="form-check-label" for="is_active">Active Page</label></div>
                    </div>
    ...
    <script src="../../Ajax/js/page.js"></script>
</body>
</html>